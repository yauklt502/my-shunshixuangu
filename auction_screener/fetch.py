...
# 竞价阶段可用的实时字段：现价/昨收/成交量/金额/买卖盘
LIVE_URL = (
    "https://push2.eastmoney.com/api/qt/stock/get"
    "?secid={secid}&ut=fa5fd1943c7b386f172d6893dbfba10b"
    "&fields=f43,f57,f58,f60,f169,f170,f46,f47,f48,f19,f20,f17,f18,f86"
)
...
def http_text(url: str, *, encoding: str = "gbk", retries: int = 3, referer: str = "https://finance.sina.com.cn/") -> str:
    last: Exception | None = None
    delay = 0.25
    for _ in range(retries):
        try:
...
def fetch_live_quote_em(code: str) -> dict[str, Any]:
    data = http_json(LIVE_URL.format(secid=secid(code)))
    p = data.get("data") or {}
    last = _em_px(p.get("f43"))
    prev = _em_px(p.get("f60"))
    open_px = _em_px(p.get("f46"))
    px = open_px if open_px > 0 else last
    vol_lots = float(p.get("f47") or 0)
    amt = float(p.get("f48") or 0)
    return {
        "code": code.split(".")[0],
        "name": p.get("f58") or "",
        "px": px,
        ...
        "amt": amt,
        "bid1_px": _em_px(p.get("f19")),
        "bid1_vol": float(p.get("f20") or 0),
        "ask1_px": _em_px(p.get("f17")),
        "ask1_vol": float(p.get("f18") or 0),
        "server_ts": int(p.get("f86") or 0),
        "source": "em",
    }
...
def fetch_sina_quotes(codes: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for i in range(0, len(codes), 80):
        chunk = codes[i : i + 80]
        text = http_text(SINA_URL.format(codes=",".join(sina_prefix(c) for c in chunk)))
        for line in text.splitlines():
            ...
            out[code] = {
                ...
                "prev": prev,
                "open": open_px,
                "vol_shares": float(parts[8] or 0),
                "vol_lots": float(parts[8] or 0) / 100.0,
                "amt": float(parts[9] or 0),
                "bid1_vol": float(parts[10] or 0),
                "bid1_px": float(parts[11] or 0),
                "ask1_vol": float(parts[20] or 0),
                "ask1_px": float(parts[21] or 0),
                "date": parts[30],
                "time": parts[31],
                "source": "sina",
            }
...
def fetch_live_quotes(codes: list[str], *, prefer: str = "sina") -> dict[str, dict[str, Any]]:
    """批量拉竞价/实时盘口。优先新浪（竞价阶段更新更稳），失败回落东财。"""
    codes = [c.split(".")[0] for c in codes]
    if prefer == "sina":
        try:
            got = fetch_sina_quotes(codes)
            if got:
                return got
    ...
    for code in codes:
        try:
            out[code] = fetch_live_quote_em(code)
        except Exception:  # noqa: BLE001
            continue
    return out
