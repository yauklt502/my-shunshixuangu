...
function Invoke-SaveScreenshot($ctx) {
  ...
  try {
    ...
    if ([string]::IsNullOrWhiteSpace($filename)) {
      $filename = "qslt_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".png"
    }
...
if (-not $listener) {
  Write-Host "Cannot bind port 3002-3003"
  exit 1
}

Write-Host "QuShi LongTou at http://127.0.0.1:$port/"
