...
浏览器打开（收藏这个地址；若刚更新过请 Ctrl+F5 强制刷新）：

**https://raw.githack.com/yauklt502/my-shunshixuangu/cursor/duanxian-xunlong-e-drive-bac5/web/index.html**

1. 网页端（githack / GitHub Pages 等静态托管）没有 `/api` 后台，会直连扶摇数据接口，点「连接测试」应显示已连接。
2. 本地：双击 `启动.bat`，打开 http://127.0.0.1:8010/ 走本机代理（专用端口，不占用顺势选股的 8000）。
3. 右上角「截图」可保存当前画面。
