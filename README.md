...
数据源与 [zt.mininas.cc](https://zt.mininas.cc/) 相同：腾讯批量行情 + 前复权日 K，名单用新浪。页面走同源代理，避免浏览器跨域失败。

**不要扫全市场。** 点「扫描主板」或「扫描创业板」：

- 主板：新浪 `sh_a`+`sz_a` 再留 `600/601/603/605/000/001/002`，非 ST（约 3000 行情）
- 创业板：新浪 `cyb` 再留 `300` 开头，非 ST（约 900 行情）
- 不扫科创 `688`、北交所、`301`、ST / 退市，**不拉** 新浪 `hs_a` 全 A（不会出现 4970）
