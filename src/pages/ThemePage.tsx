...
import { themeIdFromHash, openTheme } from "@/lib/nav";
...
export function ThemePage() {
  ...
  const [id, setId] = useState(() => themeIdFromHash());
  const home = useAsync(() => api.newGetList(common), [common]);
  const items = home.data?.Theme?.List || [];

  useEffect(() => {
    const sync = () => {
      const fromHash = themeIdFromHash();
  ...
  useEffect(() => {
    if (!id && items[0]?.ID) setId(items[0].ID);
  }, [id, items]);

  const search = useAsync(() => api.themeSearch(q, common), [q, common]);
  const detail = useAsync(
    () => (id ? api.themeInfo(id, common) : Promise.resolve(null)),
    [id, common],
  );
  ...
  return (
    <div className="grid g-sidebar">
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <Card title="题材项">
          <StateGate loading={home.loading} error={home.error} empty={!items.length}>
            <ThemeItemList items={items} activeId={id} onSelect={setId} />
          </StateGate>
        </Card>
        <Card
          title="题材库搜索"
      ...
      <Card title={detail.data?.Name || "题材详情"}>
        <StateGate loading={detail.loading} error={detail.error} empty={!id}>
          ...
          {(detail.data?.Table || []).map((block) => (
            <div key={block.Level1.ID} className="list-block">
              ...
              />
              {(block.Level2 || []).map((lv2) => (
                <div key={lv2.ID} className="list-block">
                  <div className="reason-hd">
                    <b className="muted">{lv2.Name}</b>
                  </div>
                  <Table
                    rows={lv2.Stocks || []}
                    columns={[
